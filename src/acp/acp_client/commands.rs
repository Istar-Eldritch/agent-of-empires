...
pub(super) enum ClientCmd {
    ...
    /// output_format)` tuple: sub-agents launched by a previous daemon that
    /// survived the restart. Issued by `Supervisor::attach` once, right
    /// after the connection is up, so tracking resumes instead of the panel
    /// showing them as detached.
    ResumeBackgroundTailing(Vec<(String, String, String, Option<String>)>),
    #[cfg(test)]
