...
| `agent_status_hooks` | `true` | Install status-detection hooks into the agent's config file. Codex uses the `[hooks]` table in its resolved `config.toml` (typically `~/.codex/config.toml`); other JSON-based agents use their settings JSON. OpenCode has no hook mechanism, so AoE installs a generated plugin at `~/.config/opencode/plugin/aoe-status.js` instead; it is inert outside an AoE session, and `aoe uninstall` removes it. Config-dir overrides are honored: `CODEX_HOME` (Codex), `CLAUDE_CONFIG_DIR` (Claude), `CURSOR_CONFIG_DIR` (Cursor), or `XDG_CONFIG_HOME` (OpenCode) set in the session's profile environment or in AoE's own environment redirects hooks to that directory instead of the `~/.codex` / `~/.claude` / `~/.cursor` / `~/.config/opencode` default. When disabled, status detection falls back to tmux pane content parsing. Codex is hook-first, but known hook gaps are reconciled from pane content. |
