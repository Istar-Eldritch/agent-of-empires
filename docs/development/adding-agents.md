...
- **Idle needs care with subagents.** OpenCode runs the task tool as its own session emitting its own idle, so the plugin tracks which sessions have a turn in flight and reports idle only once all of them are quiet. A session that ends on a terminal event instead (`session.error`, `session.deleted`) is dropped from that set without reporting a status, or it would pin the pane Running for the life of the process.
- **A generated plugin must register nothing without `AOE_INSTANCE_ID`.** Unlike a hook, which fires only because the user's own agent config asks for it, an auto-discovered plugin loads on *every* run of that agent on the machine. Return an empty hooks object when the variable is absent: a guard inside the work itself still costs a fork per event on runs that are none of AoE's business.
